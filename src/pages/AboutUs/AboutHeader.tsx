const AboutHeader = () => {
    return (
      <>
        <div>
          <style>
            @import url('https://fonts.googleapis.com/css2?family=Oooh+Baby&display=swap');
          </style>
        </div>
        <div style={{ backgroundColor: '#E8F4FF' }}>
            <div className="container mx-auto h-16 pt-20" >
                <h1 className="text-text-color-1 font-fjalla-one font-bold text-center text-5xl" style={{ color: '#00729D' }}>ABOUT US</h1>
            </div>
            <div className="mx-auto px-20 pt-20">
            <img src='src/assets/GalleryImages/border.png' alt='border'/>
            </div>
            <div className="mx-auto text-center pb-20">
            <h2 className="" style={{ fontFamily: "'Oooh Baby', cursive", color: '#00729D' }} >from the village, made with care</h2>
            </div>
        </div>
        {/* <div className="">
            <img src="src/assets/GalleryImages/DRIPPPPP-3.png"/>
        </div> */}

        
      </>
    )
  }
  
  
  
  export default AboutHeader;